...
def omp(x: np.ndarray, y: np.ndarray, non_zeros: int = 2) -> np.ndarray:
    ...
    active_set = []
    residual = y_vec.copy()
    max_steps = min(non_zeros, dim)

    for _ in range(max_steps):
        correlations = np.abs(x.T @ residual).reshape(-1)
        if active_set:
            correlations[np.array(active_set)] = -1
        best_idx = int(np.argmax(correlations))
        active_set.append(best_idx)

        x_active = x[:, active_set]
        w_active = np.linalg.pinv(x_active.T @ x_active) @ x_active.T @ y_vec
        residual = y_vec - x_active @ w_active

    weights[np.array(active_set)] = w_active
    return weights
...
def training_sgd(x: np.ndarray,
    ...
    batch_num = max(1, min(batch_size, num))

    for _ in range(epoch):
        permutation = rng.permutation(num)
        for start in range(0, num, batch_num):
            end = min(start + batch_num, num)
            indices = permutation[start:end]

            x_batch = x[indices]
            y_batch = y_vec[indices]
            current_batch = end - start

            grad_data = 2 * x_batch.T @ (x_batch @ w - y_batch)
            grad_reg = 2 * gamma * w
            w = w - lr * (grad_data + grad_reg)
...
def training_lasso(x: np.ndarray,
    ...
    lipschitz = 2 * (np.linalg.norm(x, ord=2) ** 2)
    if lipschitz == 0:
        return np.zeros((dim, 1))
    ...
    for _ in range(iteration):
        grad = 2 * x.T @ (x @ w - y)
        w = soft_thresholding(w - step_size * grad, gamma * step_size)
...
def training_elastic(x: np.ndarray,
    ...
    """
    dim = x.shape[1]
    x_tilde = np.vstack((x, np.sqrt(gamma2) * np.eye(dim)))
    y_tilde = np.vstack((y, np.zeros((dim, 1))))
    return training_lasso(x_tilde, y_tilde, gamma=gamma1, iteration=iteration, r_seed=r_seed)
...
def training_irls(x: np.ndarray,
    ...
    for _ in range(iteration):
        ...
        weights = np.minimum(weights, 1.0 / eps)
        weighted_x = x * weights
        a = np.einsum('ni,nj->ij', x, weighted_x)
        b = np.einsum('ni,nk->ik', x, weights * y)
