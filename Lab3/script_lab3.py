...
def kernel(x: np.ndarray, y: np.ndarray = None, k_type: str = 'rbf', bandwidth: float = 'h') -> np.ndarray:
    ...
    if k_type == 'rbf':
        dist = p_distance(x, y, p=2).T
        return np.exp(-dist / bandwidth)
    
    elif k_type == 'gate':
        dist = p_distance(x, y, p=1).T
        return (dist <= bandwidth).astype(float) / bandwidth

    elif k_type == 'triangle':
        dist = p_distance(x, y, p=1).T
        return np.maximum(bandwidth - dist, 0.0)
...
def nadaraya_watson_estimator(x_test: np.ndarray, x_train: np.ndarray, y_train: np.ndarray, k_type: str, h: float) -> np.ndarray:
    ...
    kappa = kernel(x=x_train, y=x_test, k_type=k_type, bandwidth=h)
    weights = kappa / (np.sum(kappa, axis=1, keepdims=True) + 1e-10)
...
def training_krr(x_train: np.ndarray, y_train: np.ndarray, k_type: str, h: float, tau: float = 0.1) -> np.ndarray:
    ...
    K = kernel(x=x_train, y=None, k_type=k_type, bandwidth=h)
    n = x_train.shape[0]
    alpha = np.linalg.solve(K + tau * np.eye(n), y_train)
    return alpha
...
def testing_krr(x_test: np.ndarray, x_train: np.ndarray, alpha: np.ndarray, k_type: str, h: float) -> np.ndarray:
    ...
    #  Hint: use the kernel function you implemented.
    K_test = kernel(x=x_train, y=x_test, k_type=k_type, bandwidth=h)   # (M, N)
    y_test = K_test @ alpha
    return y_test
...
def training_krr_sgd(x_train: np.ndarray,
    ...
    # full kernel matrix on training data
    K = kernel(x=x_train, y=None, k_type=k_type, bandwidth=h)   # (N, N)
    ...
    for _ in range(epoch):
        ...
        for start in range(0, num, batch_size):
            ...
            # K_B: rows selected by the mini-batch, size (B, N)
            K_B = K[batch_idx, :]
            y_B = y_train[batch_idx, :]

            # mini-batch prediction and residual
            pred_B = K_B @ alpha                      # (B, 1)
            err_B = pred_B - y_B                      # (B, 1)

            # gradient of ||y_B - K_B alpha||^2 + tau * alpha^T K alpha
            grad = 2 * (K_B.T @ err_B) + 2 * tau * (K @ alpha)
...
def training_krr_bcd(x_train: np.ndarray,
    ...
    for _ in range(epoch):
        ...
        for i in order:
            ...
            numerator = float(k_i.T @ r - tau * Ka[i, 0])
            denominator = float(k_i.T @ k_i + tau * K[i, i]) + 1e-10
